...
function install_get_default_options(): array
{
    ...
    if (empty($options)) {
        $options = [
            ...
            'title' => 'Typecho Việt Hóa',
            'description' => 'Your description here.',
            'keywords' => 'typechovh,php,blog',
            'rewrite' => 0,
            'frontPage' => 'recent',
            'frontArchive' => 0,
...
function install_get_db_drivers(): array
{
    ...
    if (\Typecho\Db\Adapter\Pdo\Mysql::isAvailable()) {
        $drivers['Pdo_Mysql'] = _t('Pdo điều khiển bộ điều hợp Mysql');
    }

    if (\Typecho\Db\Adapter\Pdo\SQLite::isAvailable()) {
        $drivers['Pdo_SQLite'] = _t('Pdo điều khiển bộ điều hợp SQLite');
    }

    if (\Typecho\Db\Adapter\Pdo\Pgsql::isAvailable()) {
        $drivers['Pdo_Pgsql'] = _t('Pdo điều khiển bộ điều hợp PostgreSql');
    }

    if (\Typecho\Db\Adapter\Mysqli::isAvailable()) {
        $drivers['Mysqli'] = _t('Bộ điều hợp chức năng gốc Mysql');
    }

    if (\Typecho\Db\Adapter\SQLite::isAvailable()) {
        $drivers['SQLite'] = _t('Bộ điều hợp chức năng gốc SQLite');
    }

    if (\Typecho\Db\Adapter\Pgsql::isAvailable()) {
        $drivers['Pgsql'] = _t('Bộ điều hợp chức năng gốc Pssql');
    }
    ...
    <div id="success" class="row typecho-page-main hidden">
        <div class="col-mb-12 col-tb-8 col-tb-offset-2">
            ...
            <div id="typecho-welcome">
                <p class="keep-word">
                    <?php _e('Bạn đã chọn sử dụng dữ liệu gốc, tên người dùng và mật khẩu của bạn giống như dữ liệu ban đầu'); ?>
                </p>
                <p class="fresh-word">
                ...
                <ul>
                    <li><a id="login-url" href=""><?php _e('Vào bảng quản trị'); ?></a></li>
                    <li><a id="site-url" href=""><?php _e('Vào xem blog'); ?></a></li>
                </ul>
                <p><?php _e('Hy vọng bạn tận hưởng Typecho Việt Hóa của mình một cách trọn vẹn nhất!'); ?></p>
            </div>
        </div>
    </div>
    <script>
...
function install_check_extension(array $extensions): ?string
{
    ...
    return _n('Thiếu phần mở rộng PHP', 'Vui lòng cài đặt ít nhất một trong các tiện ích mở rộng PHP sau trên máy chủ của bạn', count($extensions))
        . ': ' . implode(', ', $extensions);
}
...
function install_step_1()
{
    ...
    <div class="row typecho-page-main">
        <div class="col-mb-12 col-tb-8 col-tb-offset-2">
            <div class="typecho-page-title">
                <h2><?php _e('Chào mừng đến với Typecho Việt Hóa'); ?></h2>
            </div>
            <div id="typecho-welcome">
                <form autocomplete="off" method="post" action="install.php">
                    <h3><?php _e('Hướng dẫn cài đặt'); ?></h3>
                    <p class="warning">
                        <strong><?php _e('Chương trình cài đặt này sẽ tự động phát hiện xem môi trường máy chủ có đáp ứng các yêu cầu cấu hình tối thiểu hay không. Nếu không, một thông báo nhắc sẽ xuất hiện ở trên, vui lòng kiểm tra cấu hình máy chủ của bạn theo thông báo nhắc. Nếu môi trường máy chủ đáp ứng các yêu cầu, "Bắt đầu bước tiếp theo" sẽ xuất hiện nút bên dưới, nhấp vào nút này để hoàn tất cài đặt trong một bước.'); ?></strong>
                    </p>
                    <h3><?php _e('Giấy phép và Thỏa thuận'); ?></h3>
                    <ul>
                        <li><?php _e('Typecho được phát hành theo thỏa thuận <a href="http://www.gnu.org/copyleft/gpl.html">GPL</a>, chúng tôi cho phép người dùng sử dụng, sao chép, sửa đổi và phân phối bản này trong phạm vi cho phép. chương trình thỏa thuận GPL.'); ?>
                            <?php _e('Trong phạm vi của giấy phép GPL, bạn có thể tự do sử dụng nó cho mục đích thương mại và phi thương mại.'); ?></li>
                        <li><?php _e('Phần mềm Typecho Việt Hóa được hỗ trợ bởi cộng đồng của nó và nhóm phát triển cốt lõi chịu trách nhiệm phát triển chương trình bảo trì hàng ngày và xây dựng các tính năng mới.'); ?>
                            <?php _e('Nếu bạn gặp sự cố khi sử dụng, lỗi trong chương trình và các tính năng mới mà bạn mong đợi, bạn có thể giao tiếp trong cộng đồng hoặc đóng góp mã trực tiếp cho chúng tôi.'); ?>
                            <?php _e('Đối với những đóng góp nổi bật, tên của anh ấy sẽ xuất hiện trong danh sách những người đóng góp.'); ?></li>
                    </ul>

                    <p class="submit">
                        <button class="btn primary" type="submit"><?php _e('Bắt đầu bước tiếp theo &raquo;'); ?></button>
...
function install_step_1_perform()
{
    ...
    if (!$writeable) {
        $errors[] = _t('Thư mục tải lên không thể ghi được, vui lòng đặt quyền của thư mục %s trong thư mục cài đặt thành có thể ghi theo cách thủ công và tiếp tục nâng cấp', $uploadDir);
    }
            ...
            <form autocomplete="off" action="install.php" method="post">
                <ul class="typecho-option">
                    <li>
                        ...
                        <p class="description"><?php _e('Vui lòng chọn bộ điều hợp phù hợp theo loại cơ sở dữ liệu của bạn'); ?></p>
                ...
                <ul class="typecho-option">
                    <li>
                        ...
                        <input type="text" class="text" name="dbPrefix" id="dbPrefix" value="typechovh_" />
                        <p class="description"><?php _e('Tiền tố mặc định là "typechovh_"'); ?></p>
                    </li>
                </ul>
                ...
                <ul class="typecho-option typecho-option-submit">
                    <li>
                        <button id="confirm" type="submit" class="btn primary"><?php _e('Xác nhận, bắt đầu cài đặt &raquo;'); ?></button>
    ...
    <script>
        function configError(form, config, errorBox) {
            ...
            if (config.code) {
                ...
                btn.html('<?php _e('Tiếp tục cài đặt &raquo;'); ?>')
                    .attr('type', 'button')
...
function install_step_2_perform()
{
    ...
    if ($config['dbNext'] == 'config' && !install_check('config')) {
        ...
        install_raise_error(_t('Tệp cấu hình bạn tạo thủ công không được phát hiện, vui lòng kiểm tra và tạo lại tệp đó'), ['code' => $code]);
        ...
        try {
            ...
            install_raise_error(_t('Rất tiếc, không thể kết nối với cơ sở dữ liệu, vui lòng kiểm tra cấu hình cơ sở dữ liệu trước khi tiến hành cài đặt'));
        } catch (\Typecho\Db\Exception $e) {
            install_raise_error(_t('Quá trình thiết lập gặp lỗi sau: "%s". Chương trình đã bị chấm dứt, vui lòng kiểm tra thông tin cấu hình của bạn.', $e->getMessage()));
        }
        ...
        if (!install_check('config')) {
            install_raise_error(
                ...
                _t('Bạn có thể tự tạo tệp <strong>config.inc.php</strong> trong thư mục gốc của trang web và sao chép đoạn mã sau vào đó'),
                [
                'code' => $code
                ]
            );
    ...
    try {
        ...
        if (
            ...
            if ($config['dbNext'] == 'keep') {
                ...
                install_raise_error(_t('Bộ cài đặt đã kiểm tra là đã tồn tại bảng dữ liệu gốc.'), [
                    'delete' => _t('Xóa dữ liệu cũ'),
                    'keep' => _t('Sử dụng dữ liệu cũ')
                ]);
            }
        } else {
            install_remove_config_file();
            ...
            <form autocomplete="off" action="install.php" method="post">
                <ul class="typecho-option">
                    <li>
                        ...
                        <p class="description"><?php _e('Đây là đường dẫn website được chương trình tự động ghép nối, nếu sai vui lòng sửa lại'); ?></p>
                    </li>
                </ul>
                <ul class="typecho-option">
                    <li>
                ...
                <ul class="typecho-option">
                    <li>
                        <label class="typecho-label" for="userPassword"><?php _e('Mật khẩu'); ?></label>
                        <input type="password" name="userPassword" id="userPassword" class="text" />
                        <p class="description"><?php _e('Vui lòng điền mật khẩu đăng nhập, nếu để trống hệ thống sẽ tạo ngẫu nhiên mật khẩu cho bạn'); ?></p>
                    </li>
                </ul>
                <ul class="typecho-option">
                    <li>
                        <label class="typecho-label" for="userMail"><?php _e('Địa chỉ email'); ?></label>
                        <input autocomplete="new-password" type="text" name="userMail" id="userMail" class="text" />
                        <p class="description"><?php _e('Vui lòng điền vào một trong những địa chỉ email thông thường của bạn'); ?></p>
                    </li>
                </ul>
                <ul class="typecho-option typecho-option-submit">
                    <li>
...
function install_step_3_perform()
{
    ...
    $error = (new \Typecho\Validate())
        ...
        ->addRule('userUrl', 'url', _t('Vui lòng điền địa chỉ URL hợp lệ'))
        ->addRule('userName', 'required', _t('Tên người dùng là bắt buộc'))
        ->addRule('userName', 'xssCheck', _t('Vui lòng không sử dụng các ký tự đặc biệt trong tên người dùng'))
        ->addRule('userName', 'maxLength', _t('Độ dài tên người dùng vượt quá giới hạn, vui lòng không vượt quá 32 ký tự'), 32)
        ->addRule('userMail', 'required', _t('E-mail phải được điền vào'))
        ->addRule('userMail', 'email', _t('Lỗi định dạng email'))
        ->addRule('userMail', 'maxLength', _t('Độ dài của Email vượt quá giới hạn, vui lòng không vượt quá 200 ký tự'), 200)
        ->run($config);
    ...
    try {
        ...
        $installDb->query(
            $installDb->insert('table.metas')
                ->rows([
                    'name' => _t('Chuyên mục mặc định'),
                    'slug' => 'default',
                    'type' => 'category',
                    'description' => _t('Đây là một chuyên mục mặc định'),
                    'count' => 1
                ])
        );
        ...
        $installDb->query(
            $installDb->insert('table.contents')->rows([
                'title' => _t('Bạn đã cài đặt thành công mã nguồn Typecho Việt Hóa!'),
                'slug' => 'start', 'created' => \Typecho\Date::time(),
                'modified' => \Typecho\Date::time(),
                'text' => '<!--markdown-->' . _t('Xin chúc mừng! Nếu bạn thấy bài viết này có nghĩa là bạn đã cài đặt thành công mã nguồn Typecho Việt Hóa! Hãy bắt đầu tùy chỉnh blog của bạn ngay bây giờ!'),
                'authorId' => 1,
                'type' => 'post',
                'status' => 'publish',
                'commentsNum' => 1,
        ...
        $installDb->query(
            $installDb->insert('table.contents')->rows([
                ...
                'created' => \Typecho\Date::time(),
                'modified' => \Typecho\Date::time(),
                'text' => '<!--markdown-->' . _t('Trang này được tạo bởi Typecho Việt Hóa, nó chỉ là một trang thử nghiệm.'),
                'authorId' => 1,
                'order' => 0,
                'type' => 'page',
                'status' => 'publish',
        ...
        $installDb->query(
            $installDb->insert('table.comments')->rows([
                ...
                'author' => 'Typecho Việt Hóa',
                'ownerId' => 1,
                'url' => 'https://asbteam.me',
                'ip' => '127.0.0.1',
                'agent' => $options->generator,
                'text' => 'Đây là bình luận mặc định!',
                'type' => 'comment',
                'status' => 'approved',
                'parent' => 0
            ])
...
<head>
    ...
    <title><?php _e('Cài đặt Typecho Việt Hóa'); ?></title>
...
<body style="font-family: 'Dosis', sans-serif;">
    <div class="body container">
        <h1><a href="https://asbteam.me" target="_blank" class="i-logo">Typecho Việt Hóa</a></h1>
        <?php $method(); ?>
    </div>
</body>
</html>
