...
class XmlRpc extends Contents implements ActionInterface, Hook
{
    ...
    public function execute(bool $run = false)
    {
        ...
        $this->wpOptions = [
            ...
            'admin_url'        => [
                'desc'     => _t('Địa chỉ khu vực admij'),
                'readonly' => true,
                'value'    => $this->options->siteUrl . 'admin/'
            ],

            'post_thumbnail'     => [
                'desc'     => _t('Thumbnail của bài viết'),
                'readonly' => true,
                'value'    => false
            ],
            ...
            'users_can_register' => [
                'desc'     => _t('Cho phép đăng ký hay không'),
                'readonly' => false,
    ...
    public function wpGetComment(int $blogId, string $userName, string $password, int $commentId): array
    {
        ...
        if (!$comment->commentIsWriteable()) {
            throw new Exception(_t('Không có quyền truy cập vào bình luận'), 403);
        }
    ...
    public function pingbackPing(string $source, string $target): int
    {
        ...
        if ($post) {
            ...
            if ($post->allowPing) {
                ...
                throw new Exception(_t('Địa chỉ đích cấm Ping'), 49);
            }
        } else {
            throw new Exception(_t('Địa chỉ mục tiêu này không tồn tại'), 33);
        }
...
<rsd version="1.0" xmlns="http://archipelago.phrasewise.com/rsd">
    <service>
        <engineName>Typecho Việt Hóa</engineName>
