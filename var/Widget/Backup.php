...
class Backup extends BaseOptions implements ActionInterface
{
    ...
    private function import()
    {
        ...
        if (!empty($_FILES)) {
            ...
            if (0 == $file['error'] && is_uploaded_file($file['tmp_name'])) {
                ...
                Notice::alloc()->set(_t('Tải lên tệp sao lưu không thành công'), 'error');
    ...
    private function extractData($file)
    {
        ...
        if ($fileSize < $headerSize) {
            @fclose($fp);
            Notice::alloc()->set(_t('Lỗi định dạng tệp sao lưu'), 'error');
            $this->response->goBack();
        }
        ...
        while (!feof($fp) && $offset + $headerSize < $fileSize) {
            ...
            if (!$data) {
                @fclose($fp);
                Notice::alloc()->set(_t('Khôi phục dữ liệu bị lỗi'), 'error');
                $this->response->goBack();
            }
    ...
    private function importData($table, $data)
    {
        ...
        try {
            ...
            Notice::alloc()->set(_t('Đã xảy ra lỗi sau trong quá trình khôi phục: %s', $e->getMessage()), 'error');
            $this->response->goBack();
        }
    }
