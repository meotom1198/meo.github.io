...
class Feedback extends Comments implements ActionInterface
{
        ...
        if (
            ...
            if ('comment' == $callback) {
                ...
                if (!$this->content->allow('comment')) {
                    throw new Exception(_t('Xin lỗi, phản hồi cho nội dung này bị vô hiệu hóa.'), 403);
                }
                ...
                if ($this->options->commentsCheckReferer && 'false' != $this->parameter->checkReferer) {
                    ...
                    if (empty($referer)) {
                        throw new Exception(_t('Xem lại lỗi trang nguồn.'), 403);
                    }
            ...
            throw new Exception(_t('Không tìm thấy'), 404);
        }
    }
    ...
    private function comment()
    {
        ...
        $validator->addRule('author', [$this, 'requireUserLogin'], _t('Tên người dùng bạn đang sử dụng đã được đăng ký, vui lòng đăng nhập và gửi lại'));
        $validator->addRule('author', 'maxLength', _t('Tên người dùng tối đa 150 ký tự'), 150);

        if ($this->options->commentsRequireMail && !$this->user->hasLogin()) {
            $validator->addRule('mail', 'required', _t('Địa chỉ e-mail là bắt buộc'));
        }

        $validator->addRule('mail', 'email', _t('Địa chỉ email không hợp lệ'));
        $validator->addRule('mail', 'maxLength', _t('Email tối đa 150 ký tự'), 150);

        if ($this->options->commentsRequireUrl && !$this->user->hasLogin()) {
            $validator->addRule('url', 'required', _t('Trang cá nhân phải được điền vào'));
        }
        ...
        $validator->addRule('text', 'required', _t('Điền nội dung bình luận'));
    ...
    private function trackback()
    {
        ...
        if (
            ...
            /** 使用403告诉机器人 */
            throw new Exception(_t('Không gửi lại'), 403);
        }
