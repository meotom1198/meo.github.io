...
class Edit extends Options implements ActionInterface
{
    ...
    public function activate($pluginName)
    {
        ...
        if (Plugin::checkDependence($info['since'])) {
            ...
            $result = _t('<a href="%s">%s</a> không hoạt động bình thường với phiên bản Typecho Việt Hóa này', $info['homepage'], $info['title']);
        }
        ...
        if (isset($result) && is_string($result)) {
            ...
            Notice::alloc()->set(_t('Plugin được kích hoạt'), 'success');
        }
        $this->response->goBack();
    }
