...
');
            ...
            throw new Exception(_t('Không thể ghi tệp config.inc.php, vui lòng đặt quyền của nó thành có thể ghi.'));
        }
    }
        ...
        if (is_dir($uploadDir)) {
            if (!is_writeable($uploadDir)) {
                if (!@chmod($uploadDir, 0755)) {
                    throw new \Typecho\Widget\Exception(_t('Thư mục tải lên không thể ghi được, vui lòng đặt quyền của thư mục %s trong thư mục cài đặt thành có thể ghi theo cách thủ công và tiếp tục nâng cấp.', Upload::UPLOAD_DIR));
                }
            }
        } else {
            if (!@mkdir($uploadDir, 0755)) {
                throw new \Typecho\Widget\Exception(_t('Không thể tạo thư mục tải lên, vui lòng tạo thủ công thư mục %s trong thư mục cài đặt và đặt quyền của nó thành có thể ghi rồi tiếp tục nâng cấp', Upload::UPLOAD_DIR));
            }
        }
    ...
    'gpc'               =>  true
            ...
            /** 升级提示 */
            return _t('Bạn nên thực hiện <a href="http://typecho.org/upgrade/9.7.2">các bước tối ưu hóa sau</a> ngay sau khi nâng cấp lên Typecho 0.7/9.7.2 trở lên');
        }
    }
    ...
    public static function v0_7r9_9_2($db, $options)
    {
        ...
        if (false !== strpos($adapterName, 'Mysql')) {
            $db->query("ALTER TABLE  `{$prefix}contents` CHANGE  `text`  `text` LONGTEXT NULL DEFAULT NULL COMMENT  'nhân vật nội dung'", Db::WRITE);
        }
    }
