...
$(document).ready(function() {
    ...
    $('#date').mask('9999-99-99 99:99').datetimepicker({
        ...
        nextText        :   '<?php _e('Tháng tới'); ?>',
        monthNames      :   ['<?php _e('Tháng 1'); ?>', '<?php _e('Tháng 2'); ?>', '<?php _e('Tháng e'); ?>', '<?php _e('Tháng 4'); ?>',
        ...
        dayNames        :   ['<?php _e('Chủ nhật'); ?>', '<?php _e('Thứ hai'); ?>', '<?php _e('Thứ 3'); ?>',
            '<?php _e('Thứ 4'); ?>', '<?php _e('Thứ 5'); ?>', '<?php _e('Thứ 6'); ?>', '<?php _e('Thứ 7'); ?>'],
        ...
        timeOnlyTitle   :   '<?php _e('Thời gian lựa chọn'); ?>',
    ...
    if (tags.length > 0) {
        ...
        ?>, {
            ...
            noResultsText   :   '<?php _e('Nhãn này không tồn tại, nhấn enter để tạo nó'); ?>',
            prePopulate     :   tagsPre,
    ...
    function saveData(cb) {
        ...
        autoSave.text('<?php _e('Tiết kiệm'); ?>');
...
<?php if ($options->autoSave): ?>
    ...
    $('#text').bind('input propertychange', function () {
        if (!locked) {
            autoSave.text('<?php _e('Chưa được lưu'); ?>' + (lastSaveTime ? ' (<?php _e('Thời gian lưu cuối cùng'); ?>: ' + lastSaveTime + ')' : ''));
        }
...
<?php endif; ?>
    ...
    $(window).bind('beforeunload', function () {
        if (changed && !submitted) {
            return '<?php _e('Nội dung đã được thay đổi nhưng không được lưu, bạn có chắc muốn rời khỏi trang này không?'); ?>';
        }
    });
    ...
    btnPreview.click(function () {
        if (changed) {
            ...
            if (confirm('<?php _e('Nội dung sửa đổi cần được lưu trước khi có thể xem trước. Bạn có muốn lưu nội dung đó không?'); ?>')) {
                saveData(function (o) {
    ...
    $('.edit-draft-notice a').click(function () {
        if (confirm('<?php _e('Bạn có chắc chắn muốn xóa thư nháp này không?'); ?>')) {
