    ...
    <div class="container typecho-dashboard">
        ...
        <div class="row typecho-page-main">
            <div class="col-mb-12 welcome-board" role="main">
                <p><?php _e('Hiện tại có <em>%s</em> bài viết và <em>%s</em> bình luận và <em>%s</em> chuyên mục.',
                ...
                <ul id="start-link" class="clearfix">
                    <?php if ($user->pass('contributor', true)): ?>
                        ...
                        <?php if ($user->pass('editor', true) && 'on' == $request->get('__typecho_all_comments') && $stat->waitingCommentsNum > 0): ?>
                            <li>
                                <a href="<?php $options->adminUrl('manage-comments.php?status=waiting'); ?>"><?php _e('Bình luận đang chờ kiểm duyệt'); ?></a>
                        ...
                        <?php if ($user->pass('editor', true) && 'on' == $request->get('__typecho_all_comments') && $stat->spamCommentsNum > 0): ?>
                            <li>
                                <a href="<?php $options->adminUrl('manage-comments.php?status=spam'); ?>"><?php _e('Bình luận rác'); ?></a>
                        ...
                        <?php if ($user->pass('administrator', true)): ?>
                            <li><a href="<?php $options->adminUrl('themes.php'); ?>"><?php _e('Giao diện'); ?></a></li>
                            <li><a href="<?php $options->adminUrl('plugins.php'); ?>"><?php _e('Plugins'); ?></a></li>
            ...
            <div class="col-mb-12 col-tb-4" role="complementary">
                <section class="latest-link">
                    <h3><?php _e('Bài viết được xuất bản gần đây'); ?></h3>
                ...
                <section class="latest-link">
                    ...
                    <ul>
                        ...
                        <?php if ($comments->have()): ?>
                            ...
                            <?php endwhile; ?>
                        <?php else: ?>
                            <li><?php _e('Chưa có câu trả lời nào'); ?></li>
                        <?php endif; ?>
                    </ul>
                </section>
            </div>
        ...
        function applyUpdate(update) {
            if (update.available) {
                $('<div class="update-check message error"><p>'
                    + '<?php _e('Phiên bản hiện tại của bạn là %s'); ?>'.replace('%s', update.current) + '<br />'
