            ...
            <div class="col-mb-12 typecho-list">
                <div class="clearfix">
                    ...
                    <ul class="typecho-option-tabs">
                        ...
                        . (isset($request->cid) ? '?cid=' . $request->cid : '')); ?>"><?php _e('Đã duyệt'); ?></a></li>
                        ...
                        . (isset($request->cid) ? '&cid=' . $request->cid : '')); ?>"><?php _e('Chờ duyệt'); ?>
                    ...
                    <form method="get">
                        <div class="operate">
                            ...
                            <ul class="dropdown-menu">
                                ...
                                <li><a href="<?php $security->index('/action/comments-edit?do=spam'); ?>"><?php _e('Đánh dấu là spam'); ?></a></li>
                                <li><a lang="<?php _e('Bạn có chắc chắn muốn xóa những bình luận này không?'); ?>" href="<?php $security->index('/action/comments-edit?do=delete'); ?>"><?php _e('Xóa bỏ'); ?></a></li>
                            </ul>
                            <?php if('spam' == $request->get('status')): ?>
                                <button lang="<?php _e('Bạn có chắc chắn muốn xóa tất cả các bình luận spam không?'); ?>" class="btn btn-s btn-warn btn-operate" href="<?php $security->index('/action/comments-edit?do=delete-spam'); ?>"><?php _e('Xóa tất cả bình luận spam'); ?></button>
                            <?php endif; ?>
                            </div>
                        </div>
                        <div class="search" role="search">
                            ...
                            <?php endif; ?>
                            <input type="text" class="text-s" placeholder="<?php _e('Vui lòng nhập từ khóa'); ?>" value="<?php echo htmlspecialchars($request->keywords ?? ''); ?>"<?php if ('' == $request->keywords): ?> onclick="value='';name='keywords';" <?php else: ?> name="keywords"<?php endif; ?>/>
                    ...
                    <table class="typecho-list-table">
                        ...
                        ?>">
                            ...
                            <td valign="top" class="comment-body">
                                <div class="comment-date"><?php $comments->dateWord(); ?> lúc <a href="<?php $comments->permalink(); ?>"><?php $comments->title(); ?></a></div>
                                <div class="comment-content">
                                    <?php $comments->content(); ?>
                                </div> 
                                <div class="comment-action hidden-by-mouse">
                                    <?php if('approved' == $comments->status): ?>
                                    <span class="weak"><?php _e('Duyệt'); ?></span>
                                    <?php else: ?>
                                    <a href="<?php $security->index('/action/comments-edit?do=approved&coid=' . $comments->coid); ?>" class="operate-approved"><?php _e('Duyệt'); ?></a>
                                    <?php endif; ?>
                                    
                                    <?php if('waiting' == $comments->status): ?>
                                    <span class="weak"><?php _e('Chờ duyệt'); ?></span>
                                    <?php else: ?>
                                    <a href="<?php $security->index('/action/comments-edit?do=waiting&coid=' . $comments->coid); ?>" class="operate-waiting"><?php _e('Chờ duyệt'); ?></a>
                                    <?php endif; ?>
                                    
                                    <?php if('spam' == $comments->status): ?>
                                    <span class="weak"><?php _e('Bình luận spam'); ?></span>
                                    <?php else: ?>
                                    <a href="<?php $security->index('/action/comments-edit?do=spam&coid=' . $comments->coid); ?>" class="operate-spam"><?php _e('Bình luận spam'); ?></a>
                                    <?php endif; ?>
                                    ...
                                    <a lang="<?php _e('Bạn có chắc chắn muốn xóa bình luận của %s?', htmlspecialchars($comments->author)); ?>" href="<?php $security->index('/action/comments-edit?do=delete&coid=' . $comments->coid); ?>" class="operate-delete"><?php _e('Xóa bỏ'); ?></a>
                                </div>
                            </td>
                        </tr>
                        <?php endwhile; ?>
                    ...
                    <form method="get">
                        <div class="operate">
                            ...
                            <ul class="dropdown-menu">
                                <li><a href="<?php $security->index('/action/comments-edit?do=approved'); ?>"><?php _e('Duyệt'); ?></a></li>
                                <li><a href="<?php $security->index('/action/comments-edit?do=waiting'); ?>"><?php _e('Chờ duyệt'); ?></a></li>
                                <li><a href="<?php $security->index('/action/comments-edit?do=spam'); ?>"><?php _e('Soam'); ?></a></li>
                                <li><a lang="<?php _e('Bạn có chắc chắn muốn xóa những bình luận này không?'); ?>" href="<?php $security->index('/action/comments-edit?do=delete'); ?>"><?php _e('Xóa bỏ'); ?></a></li>
                            </ul>
                            <?php if('spam' == $request->get('status')): ?>
                                <button lang="<?php _e('Bạn có chắc chắn muốn xóa tất cả các bình luận spam không?'); ?>" class="btn btn-s btn-warn btn-operate" href="<?php $security->index('/action/comments-edit?do=delete-spam'); ?>"><?php _e('Xóa tất cả bình luận spam'); ?></button>
